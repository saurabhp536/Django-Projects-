...
from mgmt.models import Company,Employee
from mgmt.forms import CompanyForm,EmployeeForm, loginForm
...
3:Migrate 
4:Create superuser 
5:Super user login id and password used for login in Application
'''
...
def comp(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                form.save()#save information in database 
                return redirect("/show") #display page
            except:
                pass
    else:
        form = CompanyForm()
